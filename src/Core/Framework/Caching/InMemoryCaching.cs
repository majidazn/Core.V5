...
{
    public class InMemoryCaching : IMemoryCachingServices
    {
        private IMemoryCache _memoryCache;
        //  private readonly ILogger<InMemoryCaching> _logger;
        ...
        public InMemoryCaching(/*ILogger<MemoryCachingService> logger,*/ IMemoryCache memoryCache)
        {
            _memoryCache = memoryCache ?? throw new ArgumentNullException(nameof(_memoryCache));
     //       _logger = logger;
        }

        public async Task<T> GetOrCreateAsync<T>(string key, Func<Task<T>> factory, TimeSpan memoryCacheExpiration) where T : class
        {
            ...
            return await GetOrCreateAsync(new JsonConverter<T>(), key, factory, memoryCacheExpiration);
        }

        public async Task<string> GetOrCreateStringAsync(string key, Func<Task<string>> factory, TimeSpan memoryCacheExpiration)
        {
            return await GetOrCreateAsync(new StringConverter(), key, factory, memoryCacheExpiration);
        }

        public async Task<T> GetOrCreateAsync<T>(IConverter<T> converter, string key, Func<Task<T>> factory, TimeSpan memoryCacheExpiration)
        {
